...
5. 🔄 Auth/SSO implementation
   ...
   - ✅ 5c: Auth.js v5 (beta — needed for App Router + middleware, v4 doesn't fit) + Keycloak provider wired in `apps/portal` (`auth.ts`, route handler at `app/api/auth/[...nextauth]/route.ts`). Verified: provider registers correctly, sign-in redirects to Keycloak's real login page (branded `IAM-PORTAL`), logging in as `ayush` correctly triggers the forced password-reset screen from 5b.
