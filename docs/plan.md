...
2. ✅ **Real enforcement** — done, on branch `feat/authz-enforcement`. `POST /api/authz/check` in the portal is the policy decision point: given a Keycloak identity and a calling app's origin, it looks up the Application registered at that URL, then the caller's live status/userType/roleId/RoleAccess, and returns `{allow, reason}` (`no_account`/`disabled`/`wrong_user_type`/`no_role`/`no_grant`/`unknown_application`/`allow`). Only Employees with an active, granted Role can pass — Admins/SuperAdmins are denied by the same rule that already governs `/dashboard` (they carry no Role), a deliberate choice, not an oversight. `finance-app` is the enforcement point: `auth.ts`'s `signIn` checks once at login (`trigger: "signIn"`, denials audit-logged as `AUTHZ_DENIED` on the portal side, since finance-app has no DB to log to itself), and a new `middleware.ts` re-checks on every request to `/` (`trigger: "revalidate"`, not logged — every page view would flood the audit log for a state that hasn't changed) and redirects a now-denied session to a new `/access-denied` page instead of the old silent full access. The two apps share a secret (`AUTHZ_SERVICE_SECRET`, new in both `.env`s) since this is a service-to-service call with no browser session — checked with `crypto.timingSafeEqual`, fails closed (500) if unset rather than defaulting to open. 16 new route tests in the portal (99 total); finance-app still has none, by design (see `CLAUDE.md`). Full writeup in `docs/session-log.md`, 2026-09-12.
